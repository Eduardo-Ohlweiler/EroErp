...
const columns: TDataGridColumn<ContaReceberResponse>[] = [
...
export default function ContaReceberList() {
    const navigate        = useNavigate()
    const { showMessage } = useMessage()
    const { ask }         = useQuestion()

    const [data,          setData]          = useState<ContaReceberResponse[]>([])
    ...
    async function load(filtros: Record<string, string>, pagina = 0) {
        ...
        try {
            ...
            showMessage("error", "Erro ao carregar contas a receber")
        } finally {
            setLoading(false)
        }
    }
    ...
    function handleDelete(row: ContaReceberResponse) {
        ask(`Excluir a conta "${row.descricao ?? `#${row.id}`}"? Esta ação não pode ser desfeita.`, [
            ...
            { label: "Excluir",  variant: "delete",  onClick: async () => {
                try {
                    await api.delete(`/financeiro/contas-receber/${row.id}`)
    ...
    return (
        <TPage title="Contas a Receber" breadcrumb={["Financeiro", "Contas a Receber"]}>
            <TForm onSubmit={handleFiltrar}>
                ...
                <TFormFooter>
                    ...
                    <TFormActionsRight>
                        <TButton label="Nova Conta" variant="new" type="button" onClick={() => navigate("/financeiro/contas-receber/novo")} />
                    </TFormActionsRight>
                </TFormFooter>
            </TForm>
            ...
            <TDataGrid
                ...
                onRowClick   ={(row) => navigate(`/financeiro/contas-receber/${row.id}`)}
                actionsWidth ="120px"
                actions      ={(row) => (
                    <>
                        <TButton label="" variant="edit"
                            onClick={(e) => { e?.stopPropagation(); navigate(`/financeiro/contas-receber/${row.id}`) }} />
