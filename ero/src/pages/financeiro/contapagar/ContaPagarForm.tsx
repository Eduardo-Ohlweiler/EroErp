...
import type { ContaPagarResponse, ParcelaLocal } from "../../../types/ContaPagar"
...
export default function ContaPagarForm() {
    ...
    const [conta,         setConta]         = useState<ContaPagarResponse | null>(null)
    ...
    useEffect(() => {
        ...
        setLoading(true)
        api.get(`/financeiro/contas-pagar/${currentId}`)
            .then(res => {
                const c: ContaPagarResponse = res.data
                setConta(c)
            ...
            .catch(() => { showMessage("error", "Erro ao carregar conta"); navigate("/financeiro/contas-pagar") })
    ...
    async function handleSubmit(formData: Record<string, string>) {
        ...
        try {
            ...
            if (isEdit) {
                await api.put(`/financeiro/contas-pagar/${currentId}`, payload)
                showMessage("success", "Conta atualizada com sucesso!")
            } else {
                const res = await api.post("/financeiro/contas-pagar", payload)
    ...
    if (loading) return (
        <TPage title="Carregando..." breadcrumb={["Financeiro", "Contas a Pagar"]}>
    ...
    return (
        <TPage
            ...
            breadcrumb={["Financeiro", "Contas a Pagar", isEdit ? "Editar" : "Novo"]}
        >
