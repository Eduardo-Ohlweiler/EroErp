...
import type { TipoCobranca }               from "../../../types/TipoCobranca"
...
const columns: TDataGridColumn<TipoCobranca>[] = [
...
export default function TipoCobrancaFormList() {
    ...
    const [data,            setData]          = useState<TipoCobranca[]>([])
    ...
    async function loadGrid(pagina = page) {
        ...
        try {
            ...
            const response = await api.get(`/financeiro/tipos-cobranca?${params.toString()}`)
    ...
    function handleEdit(row: TipoCobranca) {
        setCurrentId(row.id)
        setNome(row.nome)
    ...
    async function handleSubmit(data: Record<string, string>) {
        ...
        try {
            ...
            if (currentId) {
                await api.patch(`/financeiro/tipos-cobranca/${currentId}`, payload)
                showMessage("success", "Registro atualizado com sucesso!")
            } else {
                await api.post("/financeiro/tipos-cobranca", payload)
    ...
    async function handleToggleAtivo(id: number, ativoAtual: boolean) {
        try {
            await api.patch(`/financeiro/tipos-cobranca/${id}`, { ativo: !ativoAtual })
    ...
    return (
        <TPage title="Tipos de Cobrança" breadcrumb={["Financeiro", "Auxiliares", "Tipos de Cobrança"]}>
