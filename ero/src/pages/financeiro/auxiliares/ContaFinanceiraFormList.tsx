...
const columns: TDataGridColumn<ContaFinanceira>[] = [
...
export default function ContaFinanceiraFormList() {
    ...
    async function loadGrid(pagina = page) {
        ...
        try {
            ...
            const response = await api.get(`/financeiro/contas?${params.toString()}`)
    ...
    function handleEdit(row: ContaFinanceira) {
        setCurrentId(row.id)
        setNome(row.nome)
    ...
    async function handleSubmit(data: Record<string, string>) {
        ...
        try {
            ...
            if (currentId) {
                await api.patch(`/financeiro/contas/${currentId}`, payload)
    ...
    async function handleToggleAtivo(id: number, ativoAtual: boolean) {
        try {
            await api.patch(`/financeiro/contas/${id}`, { ativo: !ativoAtual })
    ...
    return (
        <TPage title="Contas Financeiras" breadcrumb={["Financeiro", "Auxiliares", "Contas Financeiras"]}>
                    ...
                    <>
                        ...
                        <TButton
                            ...
                            onClick ={(e) => {
                                e?.stopPropagation()
                                ask(
                                    `Deseja ${row.ativo ? "bloquear" : "ativar"} a conta "${row.nome}"?`,
                                    [
