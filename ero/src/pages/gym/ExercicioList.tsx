...
interface ExercicioSummary {
  id:    number
  nome:  string
  ativo: boolean
}

const columns: TDataGridColumn<ExercicioSummary>[] = [
...
export default function ExercicioList() {
  ...
  const [data,          setData]          = useState<ExercicioSummary[]>([])
  ...
  async function load(nome = filtroNome, pagina = page) {
    ...
    try {
      ...
      const res = await api.get(`/exercicios?${params.toString()}`)
      setData(res.data.content ?? [])
      setTotalPages(res.data.totalPages ?? 1)
      setTotalElements(res.data.totalElements ?? 0)
    } catch {
      showMessage("error", "Erro ao carregar exercícios")
    } finally {
      setLoading(false)
    }
  }
  ...
  function handleExcluir(row: ExercicioSummary) {
    ask(`Excluir o exercício "${row.nome}"?`, [
      { label: "Cancelar", variant: "cancel",  onClick: () => {} },
      { label: "Excluir",  variant: "confirm", onClick: async () => {
        try {
          await api.delete(`/exercicios/${row.id}`)
          showMessage("success", "Exercício excluído com sucesso!")
          load()
        } catch {
          showMessage("error", "Erro ao excluir exercício")
        }
      }},
    ])
  }
  ...
  return (
    <TPage title="Exercícios" breadcrumb={["Gym", "Auxiliar Gym", "Exercícios"]}>
      <TForm onSubmit={handleFiltrar}>
        ...
        <TFormFooter>
          ...
          <TFormActionsRight>
            <TButton label="Novo Exercício" variant="new" type="button"
              onClick={() => navigate("/gym/exercicios/novo")} />
          </TFormActionsRight>
        </TFormFooter>
      </TForm>
      ...
      <TDataGrid
        ...
        onRowClick   ={(row) => navigate(`/gym/exercicios/${row.id}`)}
        actionsWidth ="100px"
        actions      ={(row) => (
          <>
            <TButton label="" variant="edit"
              onClick={(e) => { e?.stopPropagation(); navigate(`/gym/exercicios/${row.id}`) }} />
