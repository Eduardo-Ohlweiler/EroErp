...
const columns: TDataGridColumn<TipoEndereco>[] = [
...
export default function TipoEnderecoFormList() {
    ...
    const [data,            setData]          = useState<TipoEndereco[]>([]);
    ...
    async function loadGrid(pagina = page) {
        ...
        const response = await api.get(`/tipos/endereco?${params.toString()}`);
    ...
    function handleEdit(row: TipoEndereco) {
        setCurrentId(row.id);
        setNome(row.nome);
    ...
    async function handleSubmit(data: Record<string, string>) {
        ...
        try {
            ...
            if (currentId) {
                await api.patch(`/tipos/endereco/${currentId}`, payload);
                showMessage("success", "Registro atualizado com sucesso!");
            } else {
                await api.post("/tipos/endereco", payload);
    ...
    async function handleToggleAtivo(id: number, ativoAtual: boolean) {
        try {
            await api.patch(`/tipos/endereco/${id}`, { ativo: !ativoAtual });
            showMessage(
                "success",
