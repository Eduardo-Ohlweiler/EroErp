...
const columns: TDataGridColumn<TipoCadastro>[] = [
...
export default function TipoCadastroFormList(){
    ...
    const [data,            setData]          = useState<TipoCadastro[]>([]);
    ...
    async function loadGrid(pagina = page) {
        ...
        const response = await api.get(`/tipos/cadastro?${params.toString()}`);
    ...
    function handleEdit(row: TipoCadastro) {
        setCurrentId(row.id);
        setNome(row.nome);
    ...
    async function handleSubmit(data: Record<string, string>) {
        ...
        try {
            ...
            if (currentId) {
                await api.patch(`/tipos/cadastro/${currentId}`, payload);
                showMessage("success", "Registro atualizado com sucesso!");
            } else {
                await api.post("/tipos/cadastro", payload);
    ...
    async function handleToggleAtivo(id: number, ativoAtual: boolean) {
        try {
            await api.patch(`/tipos/cadastro/${id}`, { ativo: !ativoAtual });
            showMessage(
                "success",
    ...
    return (
        <TPage
            title       ="Tipos de Cadastros"
            breadcrumb  ={["Cadastros", "Auxiliares", "Tipos de Cadastros"]}
        >
