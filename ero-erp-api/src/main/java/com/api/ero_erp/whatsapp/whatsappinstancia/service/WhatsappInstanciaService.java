...
public class WhatsappInstanciaService {
    ...
    public WhatsappInstanciaResponseDto create(WhatsappInstanciaCreateDto dto) {
        ...
        if (whatsappInstanciaRepository.existsByInstanceNameAndClienteId(dto.instanceName(), cliente.getId()))
        ...
        if (dto.timezone() != null)            instancia.setTimezone(dto.timezone());
        if (dto.antecedenciaMinutos() != null)  instancia.setAntecedenciaMinutos(dto.antecedenciaMinutos());
        if (dto.ativo() != null)               instancia.setAtivo(dto.ativo());
    ...
    public WhatsappInstanciaResponseDto update(Long id, WhatsappInstanciaUpdateDto dto) {
        WhatsappInstancia instancia = this.findById(id);

        if (dto.nome() != null && !dto.nome().isBlank())   instancia.setNome(dto.nome());
        if (dto.token() != null && !dto.token().isBlank()) instancia.setToken(dto.token());
        if (dto.timezone() != null)                         instancia.setTimezone(dto.timezone());
        if (dto.antecedenciaMinutos() != null)              instancia.setAntecedenciaMinutos(dto.antecedenciaMinutos());
        if (dto.ativo() != null)                            instancia.setAtivo(dto.ativo());
    ...
    public void delete(Long id) {
        whatsappInstanciaRepository.delete(this.findById(id));
    }
}
